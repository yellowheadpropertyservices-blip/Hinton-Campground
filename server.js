...
function saveBookings(bookings) {
  try {
    ...
    // Auto-backup to GitHub (non-blocking)
...
const GITHUB_OWNER = 'yellowheadpropertyservices-blip';
const GITHUB_REPO = 'Hinton-Campground';
const GITHUB_FILE = 'bookings-seed.json';
let backupTimer = null;

function scheduleGitHubBackup() {
  // Debounce: wait 30 seconds after the last save before backing up
  // This prevents flooding GitHub with updates during busy periods
  if (backupTimer) clearTimeout(backupTimer);
  backupTimer = setTimeout(function() { pushToGitHub(); }, 30000);
}

function pushToGitHub() {
  if (!GITHUB_TOKEN) { console.log('No GitHub token, skipping backup'); return; }

  const bookings = loadBookings();
  // Safety: never overwrite a good backup with an empty list.
  if (!bookings.length) {
    console.error('Refusing to back up 0 bookings - seed file left untouched');
    return;
  }
  ...
  const getReq = https.request(getOptions, (getRes) => {
    ...
    getRes.on('end', () => {
      ...
      const putData = JSON.stringify({
        message: 'Auto-backup ' + bookings.length + ' bookings - ' + new Date().toISOString().slice(0,16),
        content: content,
        sha: sha || undefined
      });
      ...
      const putReq = https.request(putOptions, (putRes) => {
        let pData = '';
        putRes.on('data', chunk => pData += chunk);
        putRes.on('end', () => {
          if (putRes.statusCode === 200 || putRes.statusCode === 201) {
            console.log('GitHub backup: ' + bookings.length + ' bookings saved');
          } else {
            console.error('GitHub backup failed (' + putRes.statusCode + ')');
          }
        });
      });
      putReq.on('error', (e) => console.error('GitHub backup error:', e.message));
      putReq.write(putData);
      putReq.end();
    });
  });
  getReq.on('error', (e) => console.error('GitHub backup error:', e.message));
  getReq.end();
}
// ========== END GITHUB AUTO-BACKUP ==========

function isDateOverlap(aStart, aEnd, bStart, bEnd) {
  // Normalize to midnight local time to avoid timezone/format issues
  var a1 = new Date(String(aStart).slice(0, 10) + 'T12:00:00').getTime();
  var a2 = new Date(String(aEnd).slice(0, 10) + 'T12:00:00').getTime();
  var b1 = new Date(String(bStart).slice(0, 10) + 'T12:00:00').getTime();
  var b2 = new Date(String(bEnd).slice(0, 10) + 'T12:00:00').getTime();
  // Checkout day should NOT overlap with a new check-in day
  return a1 < b2 && b1 < a2;
}
...
// On startup, migrate from seed file if volume is empty
...
try {
  ...
  if (current.length <= 1 && fs.existsSync(SEED_FILE)) {
    ...
    console.log('Loaded ' + current.length + ' bookings from database');
  }
...
// ========== ADMIN AUTH (module scope - must outlive a single request) ==========
const ADMIN_USER = process.env.ADMIN_USER || 'admin';
const ADMIN_PASS = process.env.ADMIN_PASS || '';
const adminTokens = new Set();
if (!ADMIN_PASS) {
  console.error('WARNING: ADMIN_PASS is not set - admin login is disabled.');
  console.error('Set ADMIN_PASS in Railway > service > Variables to enable it.');
}
...
const server = http.createServer(async (req, res) => {
  ...
  // API: Site calendar - returns booked dates for a specific site in a month
  if (req.method === 'GET' && url.pathname === '/api/site-calendar') {
    ...
    if (!site || !year || !month) {
      ...
      res.end(JSON.stringify({ error: 'site, year, and month required' }));
      return;
    }
    ...
    bookings.forEach(b => {
      if (String(b.site) !== String(site) || b.campground !== campground) return;
      if (!b.checkin || !b.checkout) return;
      // Walk each day of the booking and add if within this month
      let d = new Date(b.checkin);
      const end = new Date(b.checkout);
      while (d < end) {
        const ds = d.toISOString().slice(0, 10);
        if (ds >= monthStart && ds < nextMonth && bookedDates.indexOf(ds) === -1) {
          bookedDates.push(ds);
        }
    ...
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ bookedDates: bookedDates }));
    return;
  }
  ...
  if (req.method === 'POST' && url.pathname === '/api/admin-login') {
    ...
    req.on('end', () => {
      try {
        ...
        if (d.user === ADMIN_USER && d.pass === ADMIN_PASS) {
          const token = require('crypto').randomBytes(32).toString('hex');
          adminTokens.add(token);
          // Clean old tokens after 24h
  ...
  // API: Get all bookings (admin - requires auth)
  ...
  if (req.method === 'POST' && url.pathname === '/create-payment-intent') {
    ...
    req.on('end', async () => {
      try {
        ...
        const SEASON_END = '2026-10-17';
        if (data.checkout > SEASON_END || data.checkin >= SEASON_END) {
          res.writeHead(400, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ error: 'Sorry, the campground closes October 17th. Please select dates within the camping season.' }));
          return;
        }
  ...
  if (req.method === 'POST' && url.pathname === '/api/confirm-booking') {
    ...
    req.on('end', async () => {
      try {
        ...
        // Double-check season end
        const SEASON_END = '2026-10-17';
        if (d.checkout > SEASON_END || d.checkin >= SEASON_END) {
          res.writeHead(400, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify({ error: 'Campground closes October 17th.' }));
          return;
        }
        ...
        if (d.email) {
          sendEmail(d.email, 'Booking Confirmed: ' + d.confNum + ' - Hinton Campgrounds',
            ...
            '</table>' +
            '<p style="margin-top:16px;padding:12px;background:#f2f2f2;border-radius:6px;font-size:13px"><strong>Washroom Access (keypad entry overnight):</strong><br>Men: <strong>2026</strong> &nbsp; Women: <strong>2345</strong></p>' +
            '<p>If there are any issues please contact management at <strong>780-315-9196</strong>.</p>' +
  ...
  // API: Delete/cancel a booking (requires auth)
