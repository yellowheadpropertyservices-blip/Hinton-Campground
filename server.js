...
try { if (!fs.existsSync(DATA_DIR)) fs.mkdirSync(DATA_DIR, { recursive: true }); } catch(e) { console.error('Could not create data dir:', e.message); }
...
try {
  ...
  if (current.length <= 1 && fs.existsSync(SEED_FILE)) {
    ...
    saveBookings(merged);
    console.log('Restored ' + merged.length + ' bookings from seed to volume');
  } else {
    console.log('Loaded ' + current.length + ' bookings from persistent volume');
  }
